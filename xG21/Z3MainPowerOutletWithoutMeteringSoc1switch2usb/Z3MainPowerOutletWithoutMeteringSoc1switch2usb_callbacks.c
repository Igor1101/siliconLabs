...
#define ONOFF_REPORT_DELAY_MS  20

#define SWITCH_ONOFF_ENDPOINT 	(1)
#define USB0_ONOFF_ENDPOINT 	(2)
#define USB1_ONOFF_ENDPOINT 	(3)

//#define EP1_CHANGED_BIT 	(0x01<<0)
//#define EP2_CHANGED_BIT 	(0x01<<1)
//#define EP3_CHANGED_BIT 	(0x01<<2)
...
//static uint8_t  onOffChangeMask = 0;
...
{
	emberEventControlSetInactive(onOffAttrReportEventControl);
//	emberAfFillCommandGlobalClientToServerReadAttributes(ZCL_ON_OFF_CLUSTER_ID, ZCL_ON_OFF_ATTRIBUTE_ID, );
//	emberEventControlSetDelayMS(onOffAttrReportEventControl, ONOFF_REPORT_DELAY_MS);
//	uint8_t buffer[6] = {0x0, 0x0, 0x06, 0x01, 0x1, 0x1};
//	emberAfFillCommandGlobalServerToClientReportAttributes(ZCL_ON_OFF_CLUSTER_ID, (uint8_t*)buffer, 6);
//	emberAfSetCommandEndpoints(1,1);
//	emberAfSendCommandUnicast(EMBER_OUTGOING_DIRECT, 0x0000);
	emberAfOnOffClusterPrintln(">>>>>>>>>>>>>>>>onOffAttrReportEventHandler ");

	vReportOnOffAttribute(SWITCH_ONOFF_ENDPOINT, 1);
}
...
EmberAfStatus OnOffCMDControl(uint8_t cmd)
{
	uint8_t command = cmd;
	switch(command)
	{
		...
		default:
			halToggleLed(ON_OFF_LIGHT_LED);
			break;
	}
	return EMBER_ZCL_STATUS_SUCCESS;
}

static EmberAfStatus localManuTouchControl(uint8_t cmd)
{
		...
		//--add
		    status = emberAfReadAttribute(SWITCH_ONOFF_ENDPOINT,
		      ...
		      if (newValue) {
		    	  ...
		    	  status = emberAfWriteAttribute(SWITCH_ONOFF_ENDPOINT,
		    			  	  	  	  	  	  	  ZCL_ON_OFF_CLUSTER_ID,
		      	                                  ZCL_ON_OFF_ATTRIBUTE_ID,
		      	                                  CLUSTER_MASK_SERVER,
		      	                                  (uint8_t *)&newValue,
		      ...
		      //
//		      emberAfOnOffClusterPrintln("emberEventControlSetDelayMS onOffAttrReportEventControl");
		      emberEventControlSetDelayMS(onOffAttrReportEventControl, ONOFF_REPORT_DELAY_MS);
//		      emberEventControlSetDelayMS(emberAfPluginReportingTickEventControl, ONOFF_REPORT_DELAY_MS);
	return EMBER_ZCL_STATUS_SUCCESS;
//--add
}
...
//  emberAfCorePrintln("D = %d \n", buttonPressDurationMs[1]);
  ...
  if (emberAfNetworkState() == EMBER_JOINED_NETWORK) {
	  ...
	  if(buttonPressDurationMs[1] < BUTTON_MID_MS && buttonPressDurationMs[1] > BUTTON_DEBOUNCE_MS){
		  //led
		  EmberAfStatus emStatus = localManuTouchControl(ZCL_TOGGLE_COMMAND_ID);
		  emberAfCorePrintln("%p ? %p", "EMBER_JOINED_NETWORK releaseUpEventHandler < BUTTON_MID_MS", emStatus?"FAIL":"OK");

	  }else{
		  if(buttonPressDurationMs[1] > BUTTON_LEAVE_MS){
			  //leave & join
//			  emberAfCorePrintln("%p ", "EMBER_JOINED_NETWORK  > BUTTON_LEAVE_MS  leave & join");
...
//				  emberAfCorePrintln("leave & join %p network %p: 0x%X", "Join", "start", status);
			  }
		  }
	  }

  }else{
//	  emberAfCorePrintln(" # EMBER  not JOINED_NETWORK # ");
	  if(buttonPressDurationMs[1] < BUTTON_JOIN_MS && buttonPressDurationMs[1] > BUTTON_DEBOUNCE_MS){
		  // led
		  EmberAfStatus emStatus = localManuTouchControl(ZCL_TOGGLE_COMMAND_ID);
		  emberAfCorePrintln("%p ? %p", "not in network releaseUpEventHandler  < BUTTON_JOIN_MS ", emStatus?"FAIL":"OK");
...
#else
  if(buttonPressDurationMs[1] < BUTTON_LEAVE_MS && buttonPressDurationMs[1] > BUTTON_DEBOUNCE_MS){
...
void commissioningLedEventHandler(void)
{
  ...
  if (emberAfNetworkState() == EMBER_JOINED_NETWORK) {
    ...
    emberAfReadServerAttribute(SWITCH_ONOFF_ENDPOINT,
...
void commissioningLedEventHandler(void)
{
	...
	emberAfCorePrintln("%p", "commissioningLedEventHandler");
  emberEventControlSetInactive(commissioningLedEventControl);
